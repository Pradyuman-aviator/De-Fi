...
export default function AgentProfilePage() {
    ...
    const agent = useArenaStore(s => s.agents.find(a => a.id === agentId));
    ...
    if (!agent) {
        return (
            <div className="min-h-screen flex items-center justify-center">
                <p className="text-arena-text-muted">Agent not found</p>
            </div>
        );
    }

    const color = agentColors[agent.id] || '#6366f1';
    const rank = leaderboard.find(e => e.agentId === agent.id)?.rank ?? 0;
        ...
        <div className="min-h-screen">
            ...
            <main className="max-w-5xl mx-auto px-4 py-8">
                    ...
                    <div className="flex items-start justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-arena-text-muted font-mono">{agent.type.replace('_', ' ')} Strategy</p>
                                <p className="text-sm text-arena-text-secondary mt-1 max-w-lg">{agent.description}</p>
                            </div>
                        </div>
                ...
                <div className="glass-card rounded-xl p-4 mb-6">
                    <h2 className="text-lg font-bold text-arena-text-primary mb-4">📈 P&L Performance</h2>
                ...
                <div className="glass-card rounded-xl overflow-hidden">
                    <div className="px-4 py-3 border-b border-arena-border">
                        <h2 className="text-lg font-bold text-arena-text-primary">📋 Trade History</h2>
                    </div>
                    <div className="max-h-96 overflow-y-auto">
                        {agent.trades.length === 0 ? (
                            <div className="p-8 text-center text-arena-text-muted">No trades yet — run a scenario!</div>
                        ) : (
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {[...agent.trades].reverse().map((trade, i) => (
                ...
                <div className="glass-card rounded-xl p-6 mt-6">
                    <h2 className="text-lg font-bold text-arena-text-primary mb-3">🧪 How This Strategy Works</h2>
                    <p className="text-sm text-arena-text-secondary leading-relaxed">{agent.description}</p>
                    <div className="mt-4 grid grid-cols-3 gap-4 text-center">
                        <div className="p-3 rounded-lg bg-arena-bg border border-arena-border">
                            <p className="text-lg font-bold text-arena-text-primary">{agent.winningTrades}</p>
                            <p className="text-xs text-arena-success">Winning Trades</p>
                        </div>
                        <div className="p-3 rounded-lg bg-arena-bg border border-arena-border">
                            <p className="text-lg font-bold text-arena-text-primary">{agent.losingTrades}</p>
